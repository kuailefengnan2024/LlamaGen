"""
采样器模块：autoregressive/serve/sampler.py
用途：实现高效的文本采样策略，从模型输出中采样下一个标记
功能：
1. 支持多种采样方法（贪婪、随机、束搜索等）
2. 实现温度缩放、top-k和top-p采样策略
3. 提供分类器引导采样（CFG）支持
4. 实现惩罚采样机制（如重复惩罚、频率惩罚等）
5. 支持概率和对数概率计算以及批处理采样

该模块是LlamaGen项目生成过程的关键组件，
负责将模型输出的logits转换为实际的生成内容，
通过各种采样策略控制生成内容的多样性和质量。
"""
